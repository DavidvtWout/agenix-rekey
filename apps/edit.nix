...
let
  inherit (pkgs.lib)
    concatStringsSep escapeShellArg filter hasPrefix optionalString removePrefix
    warn;

  inherit (import ../nix/lib.nix inputs)
    userFlakeDir mergedSecrets rageMasterEncrypt rageMasterDecrypt;

  relativeToFlake = filePath:
    let fileStr = builtins.unsafeDiscardStringContext (toString filePath);
    in if hasPrefix userFlakeDir fileStr then
      "." + removePrefix userFlakeDir fileStr
    else
      warn
      "Ignoring ${fileStr} which isn't a direct subpath of the flake directory ${userFlakeDir}, meaning this script cannot determine it's true origin!"
      null;
