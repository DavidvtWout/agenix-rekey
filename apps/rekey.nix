...
let
  inherit (pkgs.lib)
    attrValues concatMapStrings concatStringsSep escapeShellArg filterAttrs
    mapAttrsToList;

  inherit (import ../nix/lib.nix inputs) rageHostEncrypt rageMasterDecrypt;
  ...
  outPathFor = hostCfg:
    ...
    (toString (derivationFor hostCfg).outPath);
  drvPathFor = hostCfg:
    builtins.unsafeDiscardStringContext
    (toString (derivationFor hostCfg).drvPath);

  rekeyCommandsForHost = hostName: hostCfg:
    let
      ...
      secretsToRekey =
        filterAttrs (_: v: v.rekeyFile != null) hostCfg.config.age.secrets;
      ...
      rekeyCommand = secretName: secret:
        let secretOut = rekeyedSecrets.cachePathFor secret;
        in ''
          if [[ -e ${secretOut} ]] && [[ "$FORCE" != true ]]; then
            echo "[1;90m    Skipping[m [90m[already rekeyed] "${
              escapeShellArg hostName
            }":"${escapeShellArg secretName}"[m"
          else
            mkdir -p ${rekeyedSecrets.cacheDir}/secrets
            rm ${secretOut}.tmp &>/dev/null || true
            echo "[1;32m    Rekeying[m [90m"${
              escapeShellArg hostName
            }":[34m"${escapeShellArg secretName}"[m"
            if ! decrypt ${escapeShellArg secret.rekeyFile} ${
    ...
    in ''
      ...
      if [[ -e ${outPath} && ( "$FORCE" == true || ! -e ${outPath}/success ) ]]; then
        echo "[1;31m     Marking[m [31mexisting store path of [33m"${
          escapeShellArg hostName
        }"[31m for deletion [90m("${outPath}")[m"
    ...
    '';
in pkgs.writeShellScriptBin "agenix-rekey" ''
  set -euo pipefail
  ...
  function show_out_paths() {
    ${
      concatMapStrings (x: ''
        echo ${escapeShellArg (outPathFor x)}
      '') (attrValues nodes)
    }
  }
  ...
  function show_drv_paths() {
    ${
      concatMapStrings (x: ''
        echo ${escapeShellArg (drvPathFor x)}
      '') (attrValues nodes)
    }
  }
