...
let
  inherit (appHostPkgs.lib)
    assertMsg attrValues concatMapStrings escapeShellArg filterAttrs flip;

  # All secrets that have rekeyFile set. These will be rekeyed.
  secretsToRekey = flip filterAttrs hostConfig.age.secrets (name: secret:
    let
      hint = if secret.generator != null then
    ...
    in assert assertMsg
      ...
      "age.secrets.${name}.rekeyFile ([33m${
        toString secret.rekeyFile
      }[m) doesn't exist. ${hint}";
  ...
  cachePathFor = secret:
    ...
    in hostConfig.age.rekey.cacheDir
    + "/secrets/${identHash}-${secret.name}.age";
...
in appHostPkgs.stdenv.mkDerivation {
